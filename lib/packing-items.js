...
const ITEM_DB = {
    ...
    'Toiletries': [
        ...
        { name: 'Eye Patches', suggest: p => true },
        ...
        { name: 'Hair Styling Products', suggest: (p, g) => g === 'male' },
    ...
    'Accessories': [
        // Earrings/Bracelet/Ring now have both male and female icon art
        // (see packing-icons.js), so they're shown to everyone - the icon
        // itself adapts to the user's gender (defaulting nonbinary/unset to
        // the female variant). Chain/Necklace stayed as two separate
        // gendered items instead, per how the icons were provided - Chain
        // is strictly male-only so nonbinary users get only Necklace, not
        // both.
        { name: 'Earrings', suggest: p => true },
        { name: 'Chain',    suggest: (p, g) => g === 'male' },
        { name: 'Necklace', suggest: (p, g) => g !== 'male' },
        { name: 'Bracelet', suggest: p => true },
        { name: 'Ring',     suggest: p => true },
    ],
    'Clothing': [
        ...
        { name: 'Sarong',             suggest: (p, g) => g !== 'male' && p.sarongDest },
    ...
    'Gym': [
        ...
        { name: 'Electrolyte Packets', suggest: p => p.gym },
        { name: 'Gym Bag',         suggest: p => p.gym },
        { name: 'Ankle Socks',     suggest: p => p.gym },
    ],
    'Beach': [
        ...
        { name: 'Tote Bag',            suggest: p => p.beach },
        ...
        { name: 'UV Protection Shirt',  suggest: p => p.beach },
    ],
    'Swimming': [
    ...
    'Hiking': [
        ...
        { name: 'Band-Aids',            suggest: p => p.hiking },
    ],
    'Camping': [
    ...
    'Hot & Sunny Weather': [
        ...
        { name: 'Portable Fan', suggest: p => p.hot },
    ],
    'Snowy Weather': [
    ...
    'Night Out': [
        ...
        { name: 'Polo Shirt',            suggest: (p, g) => g !== 'female' && p.nightOut },
    ...
    'Festival': [
        ...
        { name: 'Folding Hand Fan', suggest: p => p.festival },
    ],
    'Road Trip': [
...
const SARONG_DESTS   = ['bali','thailand','sri lanka','malaysia','maldives','philippines','fiji','tanzania','kenya','seychelles','mauritius'];
...
function buildTripProfile(tripData) {
    ...
    const sarongDest     = SARONG_DESTS.some(k => dest.includes(k));
    ...
    // Live weather override: notifications.js's checkWeatherChange() polls
    // the real forecast for the current trip and caches what it actually
    // saw (hot/cold/snowy/rainy/windy) here. An actual live forecast beats
    // the static destination/season guess above - this is what makes the
    // matching weather category's items (Umbrella, warm layers, sunscreen,
    // a windbreaker, etc.) really get added to the packing list when the
    // weather genuinely shifts, not just mentioned in a notification.
    if (tripData.id) {
        try {
            const live = JSON.parse(localStorage.getItem(`pm_wx_live_${tripData.id}`) || 'null');
            if (live) {
                if (live.hot)   hot   = true;
                if (live.cold)  cold  = true;
                if (live.snowy) snowy = true;
                if (live.rainy) rainy = true;
                if (live.windy || live.rainy) windy = true;
            }
        } catch (e) {}
    }
    ...
    return { hot, cold, snowy, rainy, windy, beach, ski, hiking, camping, swimming, gym, business, nightOut, baby, themePark, festival, roadTrip, citySightseeing, dining, cruise, isInternational, springAllergy, sarongDest };
}

...
const ITEM_DB = {
    ...
    'Gym': [
        ...
        // Was `p.gym || p.hiking || p.hot` - ITEM_DB categories are
        // iterated in object-definition order and the first category whose
        // predicate matches "claims" an item's display category (see
        // getSuggestedItemKeys' seenNames dedup below). Since 'Gym' is
        // defined before 'Beach'/'Hiking', that broad hot/hiking condition
        // meant any beach trip (which always forces hot=true) or hiking
        // trip got its Water Bottle miscategorized under "Gym" even with
        // no gym activity selected at all. Scope this back to gym only -
        // 'Beach' and 'Hiking' each carry their own Water Bottle entry.
    ...
    'Beach': [
        ...
        { name: 'Water Bottle',         suggest: p => p.beach },
...
function buildTripProfile(tripData) {
    ...
    return { hot, cold, snowy, rainy, windy, beach, ski, hiking, camping, swimming, gym, business, nightOut, baby, themePark, festival, roadTrip, citySightseeing, dining, cruise, isInternational };
}

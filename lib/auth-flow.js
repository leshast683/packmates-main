...
(function () {
  ...
  function preload(page) {
    ...
    // #pmAuthFlow tells the loaded page's own scripts (lib/capacitor-nav.js
    // in particular) that this iframe is a *final destination*, not one of
    // lib/app-shell.js's tab iframes - without it, capacitor-nav.js's
    // window.self!==window.top check treats it as an embedded shell tab and
    // hides its own bottom-nav expecting a parent shell to provide one,
    // but this iframe has no such shell: the nav just disappears entirely.
    f.src = page + '#pmAuthFlow';
    frames[page] = f;
    return f;
  }

...
(function () {
  ...
  var TAB_PAGES = ['index.html', 'discover.html', 'packing-list.html', 'newTrip.html', 'profile.html'];
  ...
  tabHost.style.cssText = 'position:fixed;inset:0;z-index:10;';
  ...
  function onFrameLoaded(frame, pageName) {
    ...
    var actual = null;
    try { actual = (frame.contentWindow.location.pathname.split('/').pop() || 'index.html'); } catch (e) { /* cross-origin, shouldn't happen same-origin */ }
    ...
    if (actual && actual !== pageName) {
      delete frames[pageName];
      if (visibleKey === pageName) setActiveNav(null);
      return;
    }
  ...
  // Capture phase on the nav container fires before the tapped button's own
  // inline onclick="location.href=...", and stopping propagation there
  // keeps that handler from ever running — no real navigation happens.

// ═══════════════════════════════════════════════════════════════
//  SHARED PACKING DATA — loaded by both packing-list.js and tripPreview.html
//  profile flags: { hot, cold, snowy, rainy, windy, beach, ski,
//                   hiking, camping, swimming, gym, business,
//                   nightOut, baby, themePark, festival,
//                   roadTrip, citySightseeing, dining, cruise }
// ═══════════════════════════════════════════════════════════════
const ITEM_DB = {
    'Essentials': [
...
const WINDY_DESTS   = ['chicago','wellington','cape town','patagonia','scotland','ireland','netherlands','north sea'];
...
function getSuggestedKeys(profile, userGender, dismissed, customItems) {
    dismissed   = dismissed   || new Set();
    ...
    Object.entries(ITEM_DB).forEach(([cat, items]) => {
        items.forEach(item => {
            ...
            if (!ALWAYS_SUGGEST_CATS.has(cat) && dismissed.has(cat + '::' + item.name)) return;
            const normName = item.name.toLowerCase().trim();
            if (seenNames.has(normName)) return;
            seenNames.add(normName);
            keys.push(cat + '::' + item.name);
        });
    });
    Object.entries(customItems).forEach(([cat, names]) => {
        names.forEach(name => keys.push(cat + '::' + name));
    });
    return keys;
}
